...
class Instagrm_Feed_Widget extends WP_Widget {
	...
	function __construct() {
		parent::WP_Widget( /* Base ID */'instagrm_widget', /* Name */'Instagram Widget', array( 'description' => 'A widget to display a users instagrm feed' ) );
	}
	...
	function widget( $args, $instance ) {
		...
		echo $before_widget;
		if ( $title ){
			echo $before_title ."<span id='instagram_widget_title'>". $title ."</span>". $after_title;
		};
		...
		if(!empty($results->data)){
			...
			echo "<li><strong>The user currently does not have any images...</strong></li>";
		}
        echo "</ul>";
		echo $after_widget;
	}
	...
	function form( $instance ) {
		if ( $instance ) {
			...
			$title = __( 'Title', 'GoGetThemes' );
            $access_token = __( 'Access Token', 'GoGetThemes' );
            $user_id = __( 'User ID', 'GoGetThemes' );
			$username = __( 'Username', 'GoGetThemes' );
		...
		?>
		<p>
		<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:','GoGetThemes'); ?></label> 
		<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" />
		</p>
		<p>
		<label for="<?php echo $this->get_field_id('user_id'); ?>"><?php _e('User ID:','GoGetThemes'); ?></label> 
		...
		<p>
		<label for="<?php echo $this->get_field_id('access_token'); ?>"><?php _e('Access Token:','GoGetThemes'); ?></label> 
		...
		<p>
			<label for="<?php echo $this->get_field_id('picture_number'); ?>"><?php _e('Number of Images:','GoGetThemes'); ?></label> 
		...
		<p>
			<label for="<?php echo $this->get_field_id('picture_size'); ?>"><?php _e('Picture Size:','GoGetThemes'); ?></label> 
		...
		<p>
		<label for="<?php echo $this->get_field_id('link_images'); ?>"><?php _e('Link images to full image:','GoGetThemes'); ?></label> 
		...
		<p>
		<label for="<?php echo $this->get_field_id('show_likes'); ?>"><?php _e('Show Likes:','GoGetThemes'); ?></label> 
		...
		<p>
		<label for="<?php echo $this->get_field_id('show_caption'); ?>"><?php _e('Show Caption:','GoGetThemes'); ?></label> 
		...
		<p>
		<label for="<?php echo $this->get_field_id('debug_mode'); ?>"><?php _e('Debug Mode:','GoGetThemes'); ?></label> 
		...
		<p>If you do not have a ID or access token, please visit <a href="http://www.pinceladasdaweb.com.br/instagram/access-token/" target="_blank">Get Access token</a> to receive a valid token</p>
		<?php 
	}
